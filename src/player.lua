...
function background_music.play_for_player_force(name, music, instant)
    ...
    if not music_spec.file or (async_sent[name] and async_sent[name][music_spec.file]) then
        ...
        minetest.dynamic_add_media({
            ...
            if new_data.music ~= music or new_data.spec_idx ~= spec_idx then return end
            new_data.handle = minetest.sound_play(music_spec, {
                to_player = name,
            })
            new_data.expire_time = os.time() + music_spec.resend_time + 2
...
function background_music.decide_and_play(player, instant)
    ...
    if not start_play_gap[name] and music ~= "keep" and (data[name] and data[name].music or "null") ~= music then
